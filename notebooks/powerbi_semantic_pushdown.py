...
class DAXTranslator:
    ...
    def translate_all_measures(self, model: SemanticModel) -> Dict[str, Any]:
        ...
        for table in model.tables:
            for measure in table.measures:
                ...
                measure_lookup[measure.name] = measure
        ...
        while queue:
            ...
            sql, status = self._translate_calculate(
                measure.expression,
                measure_to_table.get(measure.name),
                measure_lookup,
            )
            if status != "success":
                sql, status = self._translate_algebra(measure.expression, produced)
            if status != "success":
                sql, status = self._translate_simple(
                    measure.expression,
                    measure_to_table.get(measure.name),
                )
    ...
    def _translate_simple(
        ...
        expr: str,
        table_name: str = None,
        conds: Optional[List[Tuple]] = None,
        ...
        if m:
            ...
            if conds:
                from_clause, where_clause = self._joins_and_where(tbl, conds)
                sql = f"SELECT SUM(`f0`.`{col}`) AS val FROM {from_clause}{where_clause}"
            else:
                sql = f"SELECT SUM(`f0`.`{col}`) AS val FROM `{tbl}` `f0`"
            return sql, "success"
        ...
        if m:
            ...
            if conds:
                from_clause, where_clause = self._joins_and_where(tbl, conds)
                sql = f"SELECT COUNT(*) AS val FROM {from_clause}{where_clause}"
            else:
                sql = f"SELECT COUNT(*) AS val FROM `{tbl}` `f0`"
            return sql, "success"
        ...
        if m:
            ...
            if conds:
                from_clause, where_clause = self._joins_and_where(tbl, conds)
                sql = f"SELECT COUNT(DISTINCT `f0`.`{col}`) AS val FROM {from_clause}{where_clause}"
            else:
                sql = f"SELECT COUNT(DISTINCT `f0`.`{col}`) AS val FROM `{tbl}` `f0`"
            return sql, "success"
        ...
        if m:
            ...
            if conds:
                from_clause, where_clause = self._joins_and_where(tbl, conds)
                sql = f"SELECT AVG(`f0`.`{col}`) AS val FROM {from_clause}{where_clause}"
            else:
                sql = f"SELECT AVG(`f0`.`{col}`) AS val FROM `{tbl}` `f0`"
            return sql, "success"
    ...
    def _translate_calculate(
        ...
        expr: str,
        table_name: str = None,
        measure_lookup: Optional[Dict[str, Measure]] = None,
    ) -> Tuple[Optional[str], str]:
        ...
        if not measure_lookup:
            measure_lookup = {}

        sql, status = self._translate_expression_with_filters(
            args[0].strip(),
            conds,
            measure_lookup,
            set(),
    ...
    def _translate_expression_with_filters(
        self,
        expr: str,
        conds: List[Tuple],
        measure_lookup: Dict[str, Measure],
        visited: Optional[Set[str]] = None,
    ) -> Tuple[Optional[str], str]:
        """Attempt to translate an expression while applying additional filters."""
        expr = expr.strip()
        ...
        if m:
            ...
            if name in visited:
                return None, "manual_required"
            measure = measure_lookup.get(name)
            if not measure:
                return None, "manual_required"
            visited = set(visited)
            visited.add(name)
            return self._translate_expression_with_filters(
                measure.expression,
                conds,
                measure_lookup,
                visited,
        ...
        simple_sql, status = self._translate_simple(expr, conds=conds)
        if status == "success" and simple_sql:
            return simple_sql, "success"

        refs = set(_measure_refs(expr))
        if refs:
            replacements = {}
            for ref in refs:
                measure = measure_lookup.get(ref)
                if not measure or ref in visited:
                    return None, "manual_required"
                new_visited = set(visited)
                new_visited.add(ref)
                sub_sql, sub_status = self._translate_expression_with_filters(
                    measure.expression,
                    conds,
                    measure_lookup,
                    new_visited,
                )
                if sub_status != "success" or not sub_sql:
                    return None, "manual_required"
                replacements[ref] = sub_sql

            rep_expr = expr
            for ref, sql in replacements.items():
                rep_expr = rep_expr.replace(f"[{ref}]", f"({sql})")

            leftover = re.sub(r"\(SELECT .*?\)", "", rep_expr, flags=re.DOTALL)
            if not self.SAFE_TOKENS_RE.match(leftover):
                return None, "manual_required"
            return rep_expr, "success"
